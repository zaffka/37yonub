...
**Содержание**
1. [ Значение по-умолчанию для перечисляемых(enum) типов](#1-значение-по-умолчанию-для-перечисляемыхenum-типов)
1. [ Тестирование производительности (benchmarking)](#2-тестирование-производительности-benchmarking)
1. [ Указатели! Указатели везде!](#3-указатели-указатели-везде)
1. [ Прерывание for/switch или for/select](#4-прерывание-forswitch-или-forselect)
1. [ Управление ошибками](#5-управление-ошибками)
1. [ Инициализация слайсов(slice)](#6-инициализация-слайсовslice)
1. [ Управление контекстом](#7-управление-контекстом)
1. [ Не использование опции -race](#8-не-использование-опции--race)
1. [ Использование имени файла в качестве параметра функций](#9-использование-имени-файла-в-качестве-параметра-функций)
1. [ Горутины и переменные в циклах](#10-горутины-и-переменные-в-циклах)
...
Приложение было основано на пакете [urfave/cli](https://github.com/urfave/cli){:target="_blank"} (неплохая библиотека для создания приложений командной строки).  Разработчик наследовал контекст от некоего общего контекста приложения. Что означало, что при остановке приложения, все горутины получат сигнал отмены.
...
**примечание [Александра](https://37yonub.ru/aboutme/)**:  
...
## 8. Не использование опции -race
Ошибка, которую я встречаю очень часто -- запуск тестов для Go-приложений без ключа `-race`.

Как было отмечено [в этом отчёте](https://blog.acolyer.org/2019/05/17/understanding-real-world-concurrency-bugs-in-go/){:target="_blank"}, несмотря на то, что *"Go был спроектирован, чтобы сделать многопоточное программирование более лёгким и менее подверженным ошибкам"*, мы всё ещё страдаем от проблем, связанных с многопоточными операциями.
...
Первый -- передать значение `i` замыканию(вложенной функции, запускаемой в горутине):
```go
ints := []int{1, 2, 3}
for _, i := range ints {
  go func(i int) {
